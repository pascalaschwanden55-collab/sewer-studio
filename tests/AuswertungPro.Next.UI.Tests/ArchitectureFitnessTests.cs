...
public sealed class ArchitectureFitnessTests
{
    ...
    public void PlayerWindow_transient_overlay_cleanup_partials_delegate_to_policy_controller_and_workflow()
    {
        var offenders = FindFileTokenOffenders(
            ...
            .Concat(FindFileTokenOffenders(
                RepoFile("src", "AuswertungPro.Next.UI", "Views", "Windows", "PlayerWindow.Coding.OverlayInput.Viewport.cs"),
                "CodingOverlayCanvasCleaner.ClearTransient",
                "CodingOverlayCleanupPolicy.ShouldRemoveTransientTag(el.Tag",
                ".OfType<FrameworkElement>()",
                "tag == OverlayTags.ToolBadge ||",
                "clearManualOverlay && tag == OverlayTags.Manual"))
            .Concat(FindFileTokenOffenders(
                RepoFile("src", "AuswertungPro.Next.UI", "Views", "Windows", "PlayerWindow.Coding.AiOverlayLifecycle.cs"),
                "_detectionAutoHideTimer",
                "DispatcherTimer?",
                "CodingOverlayCanvasCleaner.ClearAiOverlays",
        ...
        Assert.True(
            offenders.Length == 0,
            "PlayerWindow-Transient-Overlay-Cleanup soll ueber CleanupPolicy/Controller und AI-Overlay-Lifecycle-Workflow laufen:\n"
            + string.Join("\n", offenders));
    }

    [Fact]
    public void PlayerWindow_detection_overlay_cleanup_partials_delegate_to_cleanup_controller()
    {
        var cleanupTokens = new[]
        {
        ...
        Assert.True(
            offenders.Length == 0,
            "PlayerWindow-Detection-Overlay-Cleanup soll ueber DetectionOverlayCleanupController/Cleaner laufen:\n"
            + string.Join("\n", offenders));
    }
