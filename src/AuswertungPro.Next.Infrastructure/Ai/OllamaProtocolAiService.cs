...
public sealed class OllamaProtocolAiService : IProtocolAiService
{
    ...
    private readonly IProtocolAiTrainingSampleProvider _trainingSamples;
    ...
    public OllamaProtocolAiService(
        ...
        OllamaConfig config,
        string? ffmpegPath,
        IProtocolAiTrainingSampleProvider? trainingSamples = null,
        ...
        HttpClient? http = null)
    {
        ...
        _vision = new OllamaVisionFindingsService(_client, config.VisionModel);
        _trainingSamples = trainingSamples ?? NoopProtocolAiTrainingSampleProvider.Instance;
    ...
    public async Task<AiSuggestion?> SuggestAsync(AiInput input, CancellationToken ct = default)
    {
        ...
        {
            var bytes = await VideoFrameExtractor.TryExtractFramePngAsync(_ffmpegPath, input.VideoPathAbs!, input.Zeit.Value, ct);
        ...
        var trainingPool = _trainingSamples.LoadRecent(80);
    ...
    private static string BuildPrompt(
        ...
        double? meter,
        string severity,
        IReadOnlyList<ProtocolAiTrainingSample> training,
    ...
    private static IEnumerable<ProtocolAiTrainingSample> FilterTrainingSamples(
        IReadOnlyList<ProtocolAiTrainingSample> pool,
        AiInput input,
        IReadOnlyList<string> findings,
        double? meter)
    {
        if (pool.Count == 0)
            return Array.Empty<ProtocolAiTrainingSample>();
