...
public sealed class BatchPipelineService
{
    ...
    private async Task<double> GetDurationAsync(string videoPath, CancellationToken ct)
    {
        try
        {
            ...
            if (!result.IsSuccess) return 600;
            return double.TryParse(result.Stdout.Trim(), System.Globalization.NumberStyles.Float,
...
// ── DTOs ──────────────────────────────────────────────────────────────────
// Phase 5.3 vorbereitend: BatchPipelineResult / BatchFrameAnalysis /
// BatchPipelineProgress / ProtocolTarget / DamageVerification nach
...
// ProtocolTarget + DamageVerification: nach Application/Ai/Vision/VideoAnalysisModels.cs
