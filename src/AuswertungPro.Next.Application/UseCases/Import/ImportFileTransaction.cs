...
public sealed class ImportFileTransaction
{
    ...
    public ImportFileTransactionCleanupResult Cleanup()
    {
            ...
            && transactionIsDurable)
        {
            try
            {
                // Pruefen und Loeschen gehoeren in EINE Journal-Operation. Sonst kann
                // zwischen beiden Schritten ein neuer Marker erscheinen und versehentlich
                // geloescht werden.
                _journal.ClearIfOwned(FileStaging.ProjectRoot, TxId);
            }
            catch
            {
    ...
    private void WriteMarker(IReadOnlyList<PublishedFileInfo> files)
    {
        ...
        // Auch Pruefen und Schreiben muessen atomar im Journal liegen. Ein beschaedigter
        // oder fremder Marker sperrt und bleibt unveraendert erhalten.
        _journal.BeginIfMissingOrOwned(FileStaging.ProjectRoot, new ImportTransactionMarker(
            TxId,
            DateTime.UtcNow,
            _label,
