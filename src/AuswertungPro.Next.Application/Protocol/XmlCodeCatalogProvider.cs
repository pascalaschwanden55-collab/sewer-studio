...
public sealed class XmlCodeCatalogProvider : ICodeCatalogProvider
{
        ...
        Dictionary<string, int> counts)
    {
        ...
        var baseCodes = parsed.BaseCodes;
        var charExts = parsed.CharacterExtensions;
        var parameters = parsed.Parameters;
        var paramLinks = parsed.ParameterLinks;
        var listValues = parsed.ListValues;
        ...
        {
            var code = WinCanCatalogXmlParser.ExtractCloseCode(ce.CloseCode ?? "");
            ...
            var categoryPath = WinCanCatalogXmlParser.BuildCategoryPath(ce, baseCodes, classes);
            ...
            var codeParams = paramLinksByCharExt.TryGetValue(ce.PK, out var links)
                ? WinCanCatalogXmlParser.BuildParameters(links, parameters, listValues)
        ...
        foreach (var bc in baseCodes.Values.OrderBy(b => b.SortOrder))
        {
            ...
            var code = WinCanCatalogXmlParser.ExtractCloseCode(bc.CloseCode ?? "");
            if (string.IsNullOrWhiteSpace(code))
                continue;
            ...
            var categoryPath = WinCanCatalogXmlParser.BuildCategoryPath(bc, classes);
            var group = categoryPath.Count > 0 ? string.Join(" / ", categoryPath.Take(2)) : "";

            var codeParams = paramLinksByBaseCode.TryGetValue(bc.PK, out var bcLinks)
                ? WinCanCatalogXmlParser.BuildParameters(bcLinks, parameters, listValues)
                : new List<CodeParameter>();
