...
public sealed partial class XmlCodeCatalogProvider : ICodeCatalogProvider
{
    ...
    private void Load()
    {
        ...
        var doc = AuswertungPro.Next.Application.Common.SafeXmlLoader.Load(_xmlPath);
    ...
    private static Dictionary<string, FallbackText> LoadFallbackTextMap(string? path)
    {
        ...
        try
        {
            var doc = AuswertungPro.Next.Application.Common.SafeXmlLoader.Load(path);
            var root = doc.Root;
            if (root is null)
                return map;
