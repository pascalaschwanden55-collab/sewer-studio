...
/// <summary>
/// VM-unabhaengiges Regelwerk KK01–KK14 fuer die Konsistenzpruefung von Kostenpositionen.
/// Liefert ausschliesslich Warnungen — kein Save-Block.
/// Wird vom CostConsistencyCheckService (UI) aufgerufen, der vorher die ViewModels
/// in IMeasureBlockView-Instanzen konvertiert.
/// </summary>
public sealed class CostConsistencyChecker
{
    /// <summary>
    /// Prueft alle Bloecke auf KK01–KK14 und liefert die gesammelten Warnungen.
    /// </summary>
    /// <param name="blocks">Massnahmen-Bloecke als Read-Model.</param>
    /// <param name="catalog">Katalog-Dictionary (Key → CostCatalogItem).</param>
    /// <param name="templates">Vorlagen-Dictionary (MeasureId → MeasureTemplate).</param>
    /// <param name="projectStore">Optionaler Projekt-Store fuer Cross-Haltung-Pruefung (KK10).</param>
    /// <param name="currentHolding">Aktueller Haltungsname (fuer KK10-Kontext).</param>
    public IReadOnlyList<ConsistencyWarning> CheckAll(
        IReadOnlyList<IMeasureBlockView> blocks,
        ...
        string? currentHolding)
    {
        ...
        // KK14: Gesamtkosten 0 obwohl Massnahmen ausgewaehlt sind
    ...
    // ---------------------------------------------------------------------------
    // Block-Ebene (KK01–KK09, KK11–KK13, KK05)
    // ---------------------------------------------------------------------------

    private static void CheckBlock(
        IMeasureBlockView block,
        IReadOnlyDictionary<string, CostCatalogItem> catalog,
        IReadOnlyDictionary<string, MeasureTemplate> templates,
        List<ConsistencyWarning> warnings)
    {
        ...
        foreach (var line in block.Lines)
        {
            ...
            // KK01: Aktivierte Zeile hat Preis 0, obwohl im Katalog ein Preis vorhanden ist
            ...
            // KK04: Menge 0 bei aktivierter Zeile
            ...
            // KK06: Preisabweichung >50 % vom Katalogpreis
            ...
            // KK07: Preis manuell ueberschrieben (Info)
        ...
        // KK08: DN fehlt, aber ByDN-Positionen vorhanden
        ...
        // KK09: Laenge fehlt, aber Meter-Positionen vorhanden
        ...
        // KK13: Anschluss-Zeilen vorhanden aber keine Anschlussanzahl
    ...
    // ---------------------------------------------------------------------------
    // KK10: Preisabweichung ueber Haltungen hinweg (>10 %)
    // ---------------------------------------------------------------------------
    ...
    // ---------------------------------------------------------------------------
    // Hilfs-Methoden (package-internal: internal fuer Testbarkeit)
    // ---------------------------------------------------------------------------
    ...
    /// Kriterium: ItemKey enthaelt "ANSCHLUSS" oder Text enthaelt "anschluss".
    /// </summary>
    internal static bool IsConnectionLine(ICostLineView line)
    {
