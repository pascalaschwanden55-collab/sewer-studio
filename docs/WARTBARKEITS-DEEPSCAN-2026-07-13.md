...
- **Gesamtprüfung grün:** 8.649 Tests bestanden (2.485 Infrastruktur, 1.812 Pipeline, 4.290 UI und 62 ProjectModernizer). Zwei maschinengebundene Tests wurden planmäßig übersprungen. Der Release-Build endet mit 0 Fehlern und 0 Warnungen.
...
| A1-05 | Hoch → **teilweise erledigt** | `ServiceProvider.cs:50` God-Container mit IO im Ktor; als Ganzes in ~15 ViewModels injiziert | `DiagnosticsPageViewModel` ist auf `ILogTailReader` umgestellt; weitere ViewModels werden einzeln nach Nutzen/Risiko entkoppelt |
