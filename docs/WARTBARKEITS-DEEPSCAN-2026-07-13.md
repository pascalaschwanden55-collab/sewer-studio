...
- **Training-Center-Kerndienste zentralisiert (A1-07, Teilpaket):** `TrainingCenterStore`, `TrainingCenterImportService` und die KB-Diagnose werden dem Fenster übergeben statt dort erzeugt. Der zentrale Store teilt damit auch seinen Speicher-Lock über alle Fensterinstanzen. Nur weitere, bewusst erst bei Bedarf erzeugte Review-/Few-Shot-Dienste bleiben für spätere kleine Pakete offen.
...
- **Gesamtprüfung grün:** 8.654 Tests bestanden (2.485 Infrastruktur, 1.812 Pipeline, 4.295 UI und 62 ProjectModernizer). Zwei maschinengebundene Tests wurden planmäßig übersprungen. Der Release-Build endet mit 0 Fehlern und 0 Warnungen.
...
| A1-05 | Hoch → **teilweise erledigt** | `ServiceProvider.cs:50` God-Container mit IO im Ktor; als Ganzes in ~15 ViewModels injiziert | `DiagnosticsPageViewModel`, `SchattenauswertungPageViewModel` und `VsaPageViewModel` sind auf schmale Interfaces/Funktionen umgestellt; weitere ViewModels folgen einzeln |
| A1-07 | Mittel → **teilweise erledigt** | `TrainingCenterWindow.xaml.cs:83-84`, `MediaSearchWindow.xaml.cs:114` — Dienste per `new` | Die konkret gemeldeten Medien-, Store- und Importdienste sind zentralisiert; weitere lazy Review-/Few-Shot-Erzeugungen im TrainingCenter bleiben offen |
