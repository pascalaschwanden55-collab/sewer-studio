...
**Änderung:**
- Neue reine Klasse `VsaCodeValidator` (in `Domain/VsaCatalog`), Methode `IsKnownCode(string code)` → true, wenn der Code (Länge ≥ 3) im Katalog-Baum auflösbar ist. Nutzt die bestehende `VsaCodeTree`-Lookup-Logik.
...
**Test:** `VsaCodeValidator` ist reine, deterministische Logik → Tests erlaubt (CLAUDE.md). Fälle: bekannter Code (BABBB) → true; unbekannter/Müll-Code (ABC, XY, BBZ) → false; leerer/2-Zeichen-Code → false. Parse-Test: PDF-Chunk mit gemischt gültigen/Müll-Codes → nur gültige werden zu Entries.
...
- **Mehrdeutig** (> 1 Video ODER > 1 PDF): Haltungs-ID aus Video- und PDF-Dateinamen extrahieren und das **ID-passende** Element bevorzugen. Bei extrahierbarer, aber widersprüchlicher ID das schwächer belegte Element **leeren** (lieber „kein Protokoll" als falsches Protokoll).
...
- Entscheidung im Plan: `ScanProtocolOnlyAsync` ([Zeile 69-107](src/AuswertungPro.Next.Infrastructure/Ai/Training/TrainingCenterImportService.cs#L69-L107)) nutzt dieselben Picker — analog mitziehen oder bewusst unverändert lassen. Empfehlung: mitziehen (gleiche Gefahr).
