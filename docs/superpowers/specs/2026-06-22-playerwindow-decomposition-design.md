...
- **Datum:** 2026-06-22
- **Fortschreibung 2026-06-24:** Die Spec bleibt massgeblich. Aktueller Stand: `DamageMarkerController`, `QuickScanController`, Schritt 3 `CodingOverlayRenderController`, `DetectionConfirmationBuffer`, der erste `LiveDetectionController`-Schnitt, `CodingOsdMeterController`, der erste `CodingAiController`-Schnitt inklusive Health-Monitor-State und `CodingFrameReadinessController` sind umgesetzt; dazu kamen `CodingUiUpdateWorkflow`, `LiveDetectionRuntimeStartWorkflow` und mehrere UI-Adapter. Aktuelle Vermessung: `PlayerWindow*.cs` = 95 Dateien / 5014 Zeilen, `PlayerWindow.xaml` = 822 Zeilen, zusammen 5836 Zeilen. Die alte Angabe "33 Dateien / ~7.500 Zeilen" ist historisch.
...
- `dotnet test AuswertungPro.sln` → alle **1.951** Tests grün (kein Test darf brechen).
