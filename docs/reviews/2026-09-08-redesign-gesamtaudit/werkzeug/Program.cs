...
internal static partial class Program
{
    ...
    static readonly string Root = @"C:\Sewer-Studio_KI_4.5\.tmp\redesign-audit-20260908\isoliert";
    static readonly string Bin = @"C:\Sewer-Studio_KI_4.5\src\AuswertungPro.Next.UI\bin\Release\net10.0-windows10.0.19041";
    static readonly string AppXaml = @"C:\Sewer-Studio_KI_4.5\src\AuswertungPro.Next.UI\App.xaml";
    ...
    static int Main(string[] args)
    {
        ...
        try { if (args.Contains("Ressourcen")) PruefeEchteRessourcen(); else Run(args); return 0; }
            ...
            try
            {
                ...
                else if (schritt >= 5)
                {
                    ...
                    try
                    {
                        ...
                        Messe(window, zielFenster!); if (AppliedPage == "Probe") PruefeVerhalten(shell, services, window); if (AppliedPage == "Suite") await PruefeSeiten(shell, window);
    ...
    static void Messe(Window window, Window ziel)
    {
        ...
        File.WriteAllText(Path.Combine(Root, $"messung-{AppliedPage}-{AppliedTheme}.json"), JsonSerializer.Serialize(new
        {
            ...
            selected = grid?.SelectedItem?.ToString(), severityButtons = Descendants(ziel).OfType<Button>().Where(button => AppliedPage == "TrainingStudio" && button.CommandParameter is string text && new[] { "1", "2", "3", "4", "5" }.Contains(text)).Select(button => new { stufe = button.CommandParameter, button.Width, button.MinWidth, button.ActualWidth, x = button.TransformToAncestor(ziel).Transform(new Point()).X, fensterbreite = ziel.ActualWidth }).ToArray()
        }, new JsonSerializerOptions { WriteIndented = true }));
    }
}
