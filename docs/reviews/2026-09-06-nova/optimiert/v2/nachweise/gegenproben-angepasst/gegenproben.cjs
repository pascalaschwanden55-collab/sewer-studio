async (page) => {
  ...
  // v2: Esc bei Player-Entwurf fragt nach; hier bewusst verwerfen (angepasste Zeile 1)
  if(await page.locator('#winConfirm.show').count())await page.locator('#cfDiscard').click();
  ...
  r.dialogSuche=await page.evaluate(()=>{const top=window.__nova.state.dialogs.at(-1);return {fokus:document.activeElement.id,offeneDialoge:window.__nova.state.dialogs.map(d=>d.id),fokusImOberstenDialog:top?!!document.getElementById(top.id).contains(document.activeElement):null};});
  // v2: nach Abbrechen ist kein Dialog mehr offen; Strg+K bei offenem Player wird getrennt geprueft (angepasste Zeile 2)
  await page.locator('.nav[data-nav=haltungen]').click();await page.locator('#cfDiscard').click().catch(()=>{});await page.locator('#btnPlayerH').click();await page.keyboard.press('Control+k');
  r.dialogSucheImPlayer=await page.evaluate(()=>({fokus:document.activeElement.id,fokusImOberstenDialog:!!document.getElementById(window.__nova.state.dialogs.at(-1).id).contains(document.activeElement)}));await page.locator('#plDiscard').click();
  ...
  // v2: Freigabe ist nach dem Leeren gesperrt; Klick wird erzwungen, um die Abwehr zu pruefen (angepasste Zeile 3)
  await page.locator('#stRelease').click({force:true});r.training.meldung=await page.locator('#toasts').innerText();r.training.hinweis=await page.locator('#stNote').innerText();r.training.nachKlickAktiv=await page.locator('#stRelease').isEnabled();
