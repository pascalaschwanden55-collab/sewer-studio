...
- `CodingApplyController.Apply()`: Coding-Events -> `ProtocolDocument` (Klon via `ProtocolRevisionCloner`), Update/Delete-Abgleich nach `EntryId`, Schutz vor versehentlichem Leeren (Rueckfrage), in primaere Schaeden uebertragen, Trainingssamples sichern und sofort speichern.
