...
def load_samples(data_dir: str, samples_per_class: int) -> dict[str, list[Path]]:
    ...
    for class_dir in sorted(root.iterdir()):
        ...
        logger.info("  %s: %d/%d Bilder ausgewaehlt", class_dir.name, len(selected), len(images))
    return samples
    ...
    for variant_name, prompt in PROMPT_VARIANTS.items():
        ...
        for vsa_code, image_paths in samples.items():
            ...
            for img_path in image_paths:
                ...
                labels, bboxes = run_grounding(model, processor, img, prompt, args.device)
            ...
            logger.info("  %s: Recall=%.0f%% (%d/%d), Avg det=%.1f",
                        vsa_code, recall * 100, tp, tp + fn,
        ...
        logger.info(">>> %s: Overall Recall=%.1f%%, Avg=%.0fms",
                    variant_name, overall_recall * 100, avg_ms)
