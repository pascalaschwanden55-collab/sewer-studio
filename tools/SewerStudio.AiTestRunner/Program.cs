...
internal static class AiTestRunner
{
    ...
    public static async Task<int> RunAsync(string[] args)
    {
        ...
        report.SidecarYoloModel = health?.Yolo?.ActiveModelPath ?? health?.Yolo?.ResolvedModelPath;
        report.SidecarHealthError = client.LastHealthError;
        ...
        if (health is null)
        {
            ...
            await WriteReportsAsync(report, options.OutputDirectory).ConfigureAwait(false);
            Console.Error.WriteLine(report.Summary.Message);
            Console.Error.WriteLine($"Health-Fehler: {client.LastHealthError}");
            return 2;
        }
        ...
        foreach (var testCase in cases)
        {
            ...
            try
            {
                ...
                PipeAxisResult? axis = null;
                try
                {
                    axis = await client.AnalyzePipeAxisAsync(
                        new PipeAxisRequest(base64, options.PipeDiameterMm),
                        frameCts.Token).ConfigureAwait(false);
                    frameReport.PipeAxisConfidence = axis.Confidence;
                    frameReport.PipeAxisCenterX = axis.PipeCenterX;
                    frameReport.PipeAxisCenterY = axis.PipeCenterY;
                    frameReport.PipeAxisRadiusX = axis.PipeRadiusX;
                    frameReport.PipeAxisRadiusY = axis.PipeRadiusY;
                }
                catch (Exception ex) when (ex is not OperationCanceledException)
                {
                    frameReport.PipeAxisError = ex.Message;
                }
                ...
                frameReport.ViewType = result.ViewType;
                ...
                if (result.SamResponse is not null)
                {
                    ...
                    await SvgOverlayWriter.WriteAsync(
                        ...
                        pngBytes,
                        result.SamResponse,
                        PipeAxisRingScanFactory.Create(axis, result.SamResponse.ImageWidth, result.SamResponse.ImageHeight),
                        frameReport.Masks,
                        frameCts.Token).ConfigureAwait(false);
