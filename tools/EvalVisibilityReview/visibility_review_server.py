...
def make_handler(
    store: VisibilityReviewStore,
    title: str = "SewerStudio Sichtbarkeits-Review",
    question: str = "Ist der Schaden im Einzelbild wirklich sichtbar?",
    yes_text: str = "1 Ja sichtbar",
    no_text: str = "2 Nein",
    maybe_text: str = "3 Unsicher",
):
    index_html = (
        INDEX_HTML
        .replace("__TITLE__", escape(title))
        .replace("__QUESTION__", escape(question))
        .replace("__YES_TEXT__", escape(yes_text))
        .replace("__NO_TEXT__", escape(no_text))
        .replace("__MAYBE_TEXT__", escape(maybe_text))
    )
...
<footer>
  ...
  <button class="yes" onclick="labelCurrent('ja')">__YES_TEXT__</button>
  <button class="no" onclick="labelCurrent('nein')">__NO_TEXT__</button>
  <button class="maybe" onclick="labelCurrent('unsicher')">__MAYBE_TEXT__</button>
...
def run_server(
    ...
    server = ThreadingHTTPServer(
        ("127.0.0.1", port),
        make_handler(store, title, question, yes_text, no_text, maybe_text),
    )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--title", default="SewerStudio Sichtbarkeits-Review")
    parser.add_argument("--question", default="Ist der Schaden im Einzelbild wirklich sichtbar?")
    parser.add_argument("--yes-text", default="1 Ja sichtbar")
    parser.add_argument("--no-text", default="2 Nein")
    parser.add_argument("--maybe-text", default="3 Unsicher")
